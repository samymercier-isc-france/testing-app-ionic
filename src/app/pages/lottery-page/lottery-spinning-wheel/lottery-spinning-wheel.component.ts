...
export class LotterySpinningWheelComponent implements OnInit {
  ...
  winningItem: Reward = new Reward(0, null, 0);
  ...
  changeRotationFormat = (rotation: number) => {
    return 'rotate(' + (2160 - rotation) + 'deg)';
  };
  ...
  showPriceMessage = (reward: Reward) => {
    ...
    } else if (reward.rarity === 1) {
      message =
        'You won only ' + reward.amount + ' ' + reward.item.itemId + ' :(';
    } else if (reward.rarity === 2) {
      message = 'You won authorisation to play again';
    } else if (reward.rarity === 3) {
      message = 'You won ' + reward.amount + ' ' + reward.item.itemId + '!';
    } else if (reward.rarity === 4) {
      message =
        'JACKPOT !!!!! ' + reward.amount + ' ' + reward.item.itemId + '!';
    } else {
  ...
  resetWheel = () => {
    ...
    this.winningItem = new Reward(0, null, 0);
  };
}
